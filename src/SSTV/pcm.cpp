...
int open_pcm_read(struct pcm **p, const char *name) {
	if (strstr(name, "plughw:") == name || strstr(name, "hw:") == name || strstr(name, "default") == name)
		return open_pulse_read(p, name);
	if (strstr(name, ".wav") == (name + (strlen(name) - strlen(".wav"))))
		return open_wav_read(p, name);
	return 0;
}

int open_pcm_write(struct pcm **p, const char *name, int rate, int channels, float seconds) {
	if (strstr(name, "plughw:") == name || strstr(name, "hw:") == name || strstr(name, "default") == name)
		return open_pulse_write(p, name, rate, channels, seconds);
	if (strstr(name, ".wav") == (name + (strlen(name) - strlen(".wav"))))
