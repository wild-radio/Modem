...
void Robot36Decoder::decoder(std::string image_name, std::string source) {
	...
	while (demodulate( &cnt_freq, &dat_freq, &drate)) {
...
void Robot36Decoder::process_line(uint8_t *pixel, uint8_t *y_pixel, uint8_t *uv_pixel, int y_width, int uv_width, int width, int height, int n)
{
...
int Robot36Decoder::vis_code(int *reset, int *code, float cnt_freq, float drate) {
...
int Robot36Decoder::cal_header(float cnt_freq, float dat_freq, float drate) {
...
int Robot36Decoder::decode(int *reset, struct Image **img, std::string img_name, float cnt_freq, float dat_freq, float drate) {
	const int width = 320;
	...
	if (!init) {
		...
		y_width = y_len;
		uv_width = uv_len;
		y_pixel = new uint8_t[y_width * 2];
		memset(y_pixel, 0, (size_t)y_width * 2);
		uv_pixel = new uint8_t[uv_width * 2];
		memset(uv_pixel, 0, (size_t)uv_width * 2);
		init = 1;
	}
...
int Robot36Decoder::demodulate(float *cnt_freq, float *dat_freq, float *drate) {
	if (!init) {
		...
		*drate = rate * (float)factor_L / (float)factor_M;
		...
		pcm_buff = new short[channels * factor_M];
	...
	*cnt_freq = fclampf(1200.0 + std::arg(cnt_q[out] * std::conj(cnt_last)) / (2.0 * M_PI * dstep), 1100.0, 1300.0);
	*dat_freq = fclampf(1900.0 + std::arg(dat_q[out] * std::conj(dat_last)) / (2.0 * M_PI * dstep), 1500.0, 2300.0);

	if (std::abs(cnt_q[out]) > std::abs(dat_q[out])) {
		*dat_freq = 1500.0;
	} else {
		*cnt_freq = 1300.0;
	}
